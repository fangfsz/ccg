...
export function handleAuthModeChange() {
  ...
  if (transformerSelect) {
    ...
    transformerSelect.title = isCodexTokenPool
      ? t("modal.codexLockedFieldTip")
      : "";
    if (isCodexTokenPool) {
  ...
  if (fetchModelsBtn) {
    ...
    fetchModelsBtn.title = isTokenPool
      ? t("modal.fetchModelsUsingTokenPool")
      : t("modal.fetchModels");
  }
  if (urlHelp) {
    if (isCodexTokenPool) {
      ...
      urlHelp.textContent = isTokenPool
        ? t("modal.tokenPoolApiUrlHelp")
        : t("modal.apiUrlHelp");
    }
  }
  if (isCodexTokenPool) {
...
export async function saveEndpoint() {
  ...
  const existingEndpoint = config.endpoints.find(
    (ep, idx) => ep.name === name && idx !== currentEditIndex,
  );

  if (existingEndpoint) {
    showError(
      `Endpoint name "${name}" already exists. Please use a different name.`,
    );
    return;
  }
...
export async function deleteEndpoint(index) {
  try {
    ...
    const confirmed = await showConfirm(
      t("modal.confirmDelete").replace("{name}", endpointName),
    );
    if (!confirmed) {
      return;
    }
...
export async function fetchModels() {
  ...
  try {
    const resultStr = await window.go.main.App.FetchModels(
      apiUrl,
      apiKey,
      transformer,
    );
    ...
    if (result.success && result.models && result.models.length > 0) {
      ...
      showNotification(
        t("modal.fetchModelsSuccess").replace("{count}", result.models.length),
        "success",
      );
    } else {
      const msg = result.message?.includes("no_models_found")
        ? t("modal.fetchModelsEmpty")
        : t("modal.fetchModelsFailed");
...
// Render model dropdown with search
function renderModelDropdown(models, dropdown, input, searchTerm = "") {
  dropdown.innerHTML = "";

  // Create search input if not exists
  let searchInput = dropdown.parentNode.querySelector(".model-search-input");
  if (!searchInput) {
    searchInput = document.createElement("input");
    searchInput.type = "text";
    searchInput.className = "model-search-input";
    searchInput.placeholder = t("modal.modelSearchPlaceholder");
    dropdown.parentNode.insertBefore(searchInput, dropdown);

    searchInput.addEventListener("input", (e) => {
      renderModelDropdown(fetchedModels, dropdown, input, e.target.value);
    });
  ...
  if (filteredModels.length === 0) {
    ...
    noResults.className = "model-dropdown-item model-no-results";
    noResults.textContent = t("modal.modelNoResults");
...
export async function savePort() {
  ...
  try {
    ...
    showNotification(
      t("modal.portUpdateFailed").replace("{error}", error),
      "error",
    );
  }
}
...
const CHAT_QRCODE_URL =
  "https://gitee.com/hea7en/images/raw/master/group/chat.png";
...
export async function showWelcomeModal() {
  ...
  try {
    ...
    document.querySelector("#welcomeModal .modal-header h2").textContent = t(
      "welcome.titleWithVersion",
  ...
  try {
    ...
    const base64Data =
      await window.go.main.App.FetchImageAsBase64(urlWithTimestamp);
    if (base64Data) {
...
export function closeWelcomeModal() {
  ...
  if (dontShowAgain) {
    localStorage.setItem("ccg_welcomeShown", "true");
  }
...
export async function showChangelogIfNewVersion() {
  ...
  const lastVersion = localStorage.getItem("ccg_lastVersion");

  if (lastVersion && lastVersion !== currentVersion) {
    setTimeout(() => showChangelogModal(), 600);
  }
  localStorage.setItem("ccg_lastVersion", currentVersion);
}

export function showWelcomeModalIfFirstTime() {
  const hasShown = localStorage.getItem("ccg_welcomeShown");
  if (!hasShown) {
    setTimeout(() => {
      showWelcomeModal();
    }, 500);
...
export async function testEndpointHandler(index, buttonElement) {
  ...
  const endpointItem =
    buttonElement.closest(".endpoint-item") ||
    buttonElement.closest(".endpoint-item-compact");
  const endpointName = endpointItem ? endpointItem.dataset.name : null;

  // 简洁视图：同时更新 moreBtn
  const moreBtn = endpointItem
    ? endpointItem.querySelector('[data-action="more"]')
    : null;
  if (moreBtn) {
