...
export async function checkForUpdates(silent = false) {
  ...
  try {
    ...
    if (!result.success) {
      if (!silent) {
        showNotification(
          t("update.checkFailed") + ": " + result.error,
          "error",
        );
      }
      return;
...
function showUpdateNotification(info) {
  ...
  if (navigator.platform.indexOf("Win") === -1) {
    const title = "ccg " + t("update.newVersionAvailable");
    const message = t("update.latestVersion") + ": " + info.latestVersion;
    SendUpdateNotification(title, message).catch((err) =>
  ...
  document
    .getElementById("btn-download-update")
    .addEventListener("click", () => {
      startDownload(info);
    });

  document
    .getElementById("btn-skip-version")
...
function updateProgressBar(progress) {
  ...
  if (progressFill && progressText) {
    ...
    progressText.textContent =
      t("update.downloading") + " " + Math.round(progress.progress) + "%";
  }
}
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      try {
        ...
        if (result.success) {
          if (result.exePath) {
            ...
            if (applyResult.success) {
              ...
              showNotification(
                t("update.applyFailed") + ": " + applyResult.error,
                "error",
              );
              btn.disabled = false;
          ...
          showNotification(
            t("update.installFailed") + ": " + result.error,
            "error",
          );
          btn.disabled = false;
          btn.textContent = btnText;
        ...
        showNotification(
          t("update.installFailed") + ": " + error.message,
          "error",
        );
        btn.disabled = false;
        btn.textContent = btnText;
...
function showError(errorMsg, info) {
  ...
  document
    .getElementById("btn-retry-download")
    .addEventListener("click", () => {
      if (info) {
        progressContainer.innerHTML = `
                <div class="update-progress-bar">
...
async function saveUpdateSettings() {
  try {
    const checkIntervalSelect = document.getElementById("check-interval");
    const checkInterval = checkIntervalSelect
      ? parseInt(checkIntervalSelect.value)
      : 24;
