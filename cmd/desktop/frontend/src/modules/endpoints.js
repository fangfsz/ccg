...
export function getEndpointTestStatus(endpointName) {
  try {
    const statusMap = JSON.parse(
      localStorage.getItem(ENDPOINT_TEST_STATUS_KEY) || "{}",
    );
...
function copyToClipboard(text, button) {
  navigator.clipboard.writeText(text).then(() => {
    ...
    button.innerHTML =
      '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"><path d="M20 6L9 17l-5-5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    setTimeout(() => {
...
export async function renderEndpoints(endpoints) {
  ...
  if (endpointCountEl) {
    if (isFiltered) {
      endpointCountEl.textContent = `(${filteredEndpoints.length} / ${endpoints.length})`;
    } else {
...
function ensureTokenPoolModal() {
  ...
  modal
    .querySelector("#tokenPoolCloseBtn")
    .addEventListener("click", closeModal);
  modal
    .querySelector("#tokenPoolImportBtn")
    .addEventListener("click", handleTokenPoolImport);
  modal
    .querySelector("#tokenPoolImportFilesBtn")
    .addEventListener("click", handleTokenPoolFileImport);
  modal
    .querySelector("#tokenPoolRefreshBtn")
    .addEventListener("click", async () => {
      await loadTokenPoolData(tokenPoolCurrentIndex);
    });
  modal
    .querySelector("#tokenPoolRateRefreshBtn")
    .addEventListener("click", async () => {
      await refreshTokenPoolRateLimits(tokenPoolCurrentIndex);
    });
  modal
    .querySelector("#tokenPoolProxySaveBtn")
    .addEventListener("click", async () => {
      await saveTokenPoolProxySetting();
    });
  modal
    .querySelector("#tokenPoolProxyClearBtn")
    .addEventListener("click", async () => {
...
async function saveTokenPoolProxySetting(clear = false) {
  ...
  try {
    ...
    input.value = proxyUrl;
    showNotification(
      proxyUrl ? "Codex proxy updated" : "Codex proxy cleared",
      "success",
    );
  } catch (error) {
    const message = error?.message || String(error);
...
function maskTokenPoolEmail(email) {
  ...
  const localMasked =
    local.length <= 2 ? `${local[0] || ""}*` : `${local[0]}*${local.slice(-2)}`;
...
function ensureTokenPoolErrorModal() {
  ...
  modal
    .querySelector("#tokenPoolErrorCloseBtn")
    ?.addEventListener("click", closeModal);
  return modal;
}
...
function ensureTokenPoolUsageModal() {
  ...
  modal
    .querySelector("#tokenPoolUsageCloseBtn")
    ?.addEventListener("click", closeModal);
  return modal;
}
...
function showTokenPoolUsageDialog(label, usage) {
  ...
  if (!usage) {
    ...
    const updatedAt = usage.updatedAt
      ? formatTokenPoolTime(usage.updatedAt)
      : "-";
    body.innerHTML = `
...
function showTokenPoolUpdateTokenDialog() {
  return new Promise((resolve) => {
    ...
    modal
      .querySelector("#tokenPoolUpdateOk")
      ?.addEventListener("click", handleSubmit);
    modal
      .querySelector("#tokenPoolUpdateCancel")
...
function renderTokenPoolStatus(status, enabled = true, rateLimits = null) {
  ...
  const normalized =
    rateStatus && rateStatus !== "ok" ? rateStatus : status || "active";
  const colors = {
    active: "#10b981",
    expiring: "#f59e0b",
    need_refresh: "#f97316",
...
function renderTokenPoolStats(stats = {}) {
  ...
  const parts = items.map(([label, value]) => {
    const highlight =
      value > 0 && label !== "Total" && label !== "Active"
        ? " token-pool-stat-alert"
        : "";
...
function renderTokenPoolRateLimits(rateLimits) {
  ...
  const updatedAt = rateLimits.updatedAt
    ? formatTokenPoolTime(rateLimits.updatedAt)
    : "-";
  ...
  const creditText = credits.unlimited
    ...
    : credits.balance
      ...
      : credits.hasCredits
        ? "has credits"
        : "";
  const primaryReset = primary.resetsAt
    ? formatTokenPoolTime(new Date(primary.resetsAt * 1000).toISOString())
    : "";
  const secondaryReset = secondary.resetsAt
    ? formatTokenPoolTime(new Date(secondary.resetsAt * 1000).toISOString())
    : "";
  const metaParts = [];
  if (creditText) {
  ...
  if (primaryReset) {
    detailLines.push(
      `reset ${primaryLabel || "primary"} ${primaryReset}`.trim(),
    );
  }
  if (secondaryReset) {
    detailLines.push(
      `reset ${secondaryLabel || "secondary"} ${secondaryReset}`.trim(),
    );
  }
  if (creditText) {
...
async function loadTokenPoolData(index) {
  ...
  credentials.forEach((cred) => {
    ...
    const rateErr =
      cred.rateLimits &&
      cred.rateLimits.status &&
      cred.rateLimits.status !== "ok"
  ...
  bodyEl.querySelectorAll(".token-pool-toggle-action").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        await window.go.main.App.SetEndpointCredentialEnabled(
          tokenPoolCurrentIndex,
          credentialID,
          targetEnabled,
        );
        showNotification(
          `Credential ${targetEnabled ? "enabled" : "disabled"}`,
          "success",
        );
        await loadTokenPoolData(tokenPoolCurrentIndex);
  ...
  bodyEl.querySelectorAll(".token-pool-activate").forEach((button) => {
    button.addEventListener("click", async () => {
      try {
        await window.go.main.App.ActivateEndpointCredential(
          tokenPoolCurrentIndex,
  ...
  bodyEl.querySelectorAll(".token-pool-error-view").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const errorText = errorId
        ? tokenPoolErrorCache.get(String(errorId)) || ""
        : "";
  ...
  bodyEl.querySelectorAll(".token-pool-usage").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const accountText =
        row?.querySelector("td code")?.textContent?.trim() || "";
  ...
  bodyEl.querySelectorAll(".token-pool-rate-refresh").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const accountText =
        row?.querySelector("td code")?.textContent?.trim() || "";
      const label = accountText
        ? `${accountText} (#${credentialID})`
        : `#${credentialID}`;
      const modal = ensureTokenPoolModal();
      try {
        setTokenPoolHint(modal, `Refreshing rate limits for ${label}...`);
        const raw = await window.go.main.App.FetchCodexRateLimitsForCredential(
          tokenPoolCurrentIndex,
          credentialID,
        );
        ...
        await loadTokenPoolData(tokenPoolCurrentIndex);
        const refreshedRow = modal
          .querySelector(`.token-pool-rate-refresh[data-id="${credentialID}"]`)
          ?.closest("tr");
        const rateMain = refreshedRow
          ?.querySelector(".token-pool-rate-main")
          ?.textContent?.trim();
        const rateStatus = refreshedRow
          ?.querySelector(".token-pool-rate-status")
          ?.textContent?.trim();
        const rateSummary = rateMain || rateStatus || "-";
        showNotification(
          `Rate limits refreshed for ${label}: ${rateSummary} (${detail})`,
          "success",
        );
        setTokenPoolHint(
          modal,
  ...
  bodyEl.querySelectorAll(".token-pool-refresh-token").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        const raw = await window.go.main.App.RefreshEndpointCredential(
          tokenPoolCurrentIndex,
          credentialID,
        );
        const result = parseAppJSON(raw);
  ...
  bodyEl.querySelectorAll(".token-pool-delete").forEach((button) => {
    button.addEventListener("click", async () => {
      try {
        ...
        await window.go.main.App.DeleteEndpointCredential(
          tokenPoolCurrentIndex,
          credentialID,
        );
...
async function refreshTokenPoolRateLimits(index) {
  ...
  try {
    ...
    const data = result.data || {};
    showNotification(
      `Rate limits refreshed: ${data.updated || 0} updated, ${data.failed || 0} failed, ${data.skipped || 0} skipped`,
      "success",
    );
...
async function handleTokenPoolImport() {
  ...
  const overwrite =
    modal.querySelector("#tokenPoolOverwrite")?.checked === true;
  ...
  try {
    const resultRaw = await window.go.main.App.ImportEndpointCredentials(
      tokenPoolCurrentIndex,
      raw,
      overwrite,
    );
...
async function handleTokenPoolFileImport() {
  ...
  try {
    ...
    const resultRaw =
      await window.go.main.App.ImportEndpointCredentialsFromFiles(
        tokenPoolCurrentIndex,
        overwrite,
      );
    const result = parseAppJSON(resultRaw);
...
function copyEndpointConfig(index, button) {
  ...
  if (index < 0 || index >= allEndpoints.length) {
    ...
    showNotification(
      t("endpoints.cloneFailed") +
        ": " +
        (t("endpoints.invalidIndex") || `Invalid index ${index}`),
      "error",
    );
    return;
  }
  ...
  if (endpoint) {
    ...
    showNotification(
      t("endpoints.cloned") || "Endpoint cloned successfully",
      "success",
    );
    ...
    if (typeof window.showAddEndpointModalWithPreset === "function") {
      try {
        ...
        showNotification(
          t("endpoints.cloneFailed") + ": " + error.message ||
            `Failed to clone endpoint: ${error.message}`,
          "error",
        );
      }
    } else {
      ...
      showNotification(
        t("endpoints.cloneFailed") +
          ": " +
          t("endpoints.functionUnavailable") ||
          "Failed to clone endpoint: Function not available",
        "error",
      );
    }
  } else {
    ...
    showNotification(
      t("endpoints.cloneFailed") +
        ": " +
        (t("endpoints.noEndpointAtIdxWithIndex") ||
          `No endpoint found at index ${index}`),
      "error",
    );
  }
}
...
function setupDragAndDrop(item, container) {
  ...
  item.addEventListener("drop", async (e) => {
    ...
    if (draggedElement && draggedElement !== item) {
      ...
      const orderChanged = !currentOrder.every(
        (name, idx) => name === newOrder[idx],
      );
...
function renderCompactView(
  ...
  sortedEndpoints.forEach(({ endpoint: ep, originalIndex: index, stats }) => {
    ...
    // 截断 URL 显示
    const displayUrl =
      ep.apiUrl.length > 40 ? ep.apiUrl.substring(0, 40) + "..." : ep.apiUrl;
    const displayModel =
      model && model.length > 15 ? model.substring(0, 15) + "..." : model || "";
    const displayRemark =
      ep.remark && ep.remark.length > 12
        ? ep.remark.substring(0, 12) + "..."
        : ep.remark || "";
...
function updateItemPositions(container, draggedElement, placeholder) {
  const allItems = Array.from(
    container.querySelectorAll(".endpoint-item-compact"),
  );
  ...
  while (currentNode) {
    if (
      ...
      currentNode.classList.contains("endpoint-item-compact")
    ) {
      targetIndex++;
    }
...
// 根据鼠标位置移动占位符
function movePlaceholderByMousePosition(
  e,
  container,
  draggedElement,
  dragPlaceholder,
  ...
  if (closestItem) {
    ...
    if (
      targetPosition !== dragPlaceholder &&
      targetPosition !== dragPlaceholder.nextSibling
    ) {
