export default {
  ...
  modal: {
    ...
    tokenPoolApiUrlHelp:
      "Token Pool 仅负责认证，API 地址与转换器行为保持与端点配置一致",
    codexTokenPoolApiUrlHelp:
      "Codex Token Pool 使用固定上游 https://chatgpt.com/backend-api/codex 和固定转换器 OpenAI2（Responses API）",
    codexLockedFieldTip: "Codex Token Pool 模式下此字段为固定值",
    apiKey: "API 密钥",
    apiKeyPlaceholder: "例如：sk-ant-api03-...",
    togglePassword: "显示/隐藏密钥",
    transformer: "转换器",
    transformerHelp: "选择此端点的 API 格式，auto 会根据中转方案和模型自动选择",
    transformerOptionAuto: "自动选择 (推荐)",
    transformerOptionPassthrough: "直接透传 (Passthrough)",
    ...
    providerType: "中转方案",
    providerTypeHelp: "选择端点的中转方案类型，native表示原生API",
    providerTypeNative: "原生 (Native)",
    providerTypeOneAPI: "OneAPI",
    providerTypeNewAPI: "NewAPI",
    providerTypeSub2API: "Sub2API",
    providerTypeCliproxyAPI: "CliproxyAPI",
    customPath: "自定义路径",
    customPathPlaceholder: "例如：/v1/chat/completions",
    customPathHelp: "可选：自定义API路径，会覆盖自动选择的路径",
    cancel: "取消",
    save: "保存",
    invalidFormat: "格式无效",
  ...
  welcome: {
    title: "欢迎使用 ccg！",
    titleWithVersion: "👋 欢迎使用 ccg v{version}",
    message: "ccg 是一个为 Claude Code 设计的智能 API 端点轮询代理工具",
    features: "功能特性",
  ...
  settings: {
    ...
    themeAuto: "自动模式",
    themeAutoHelp:
      "自动模式：根据时间在浅色和深色主题之间自动切换（7:00-19:00 使用浅色主题）",
    ...
    notificationHelp:
      "当 Claude Code 任务完成时发送通知提醒（ 注：修改后必须重启终端生效 ）",
  ...
  backup: {
    ...
    errors: {
      ...
      backup_s3_endpoint_invalid:
        "S3 Endpoint 无效（请填写 host:port 或 https://host:port）",
      backup_s3_endpoint_path_not_supported:
        "S3 Endpoint 不支持包含路径，请将路径部分填写到 Prefix",
  ...
  update: {
    ...
    install_instructions_windows:
      "1. 关闭当前 ccg 应用<br>2. 运行新版本的 ccg.exe",
    install_instructions_macos:
      "1. 关闭当前 ccg 应用<br>2. 将新版本的 ccg.app 拖到应用程序文件夹",
    install_instructions_linux:
      "1. 关闭当前 ccg 应用<br>2. 运行新版本的 ccg 可执行文件",
  ...
  terminal: {
    ...
    selectTerminalHelpGemini: "选择用于启动 Gemini CLI 的终端程序",
    projectDirs: "项目目录",
    ...
    selectCliType: "选择 CLI 工具",
    configStatus: "配置状态",
    configApplied: "配置已应用",
    configNotApplied: "配置未应用",
    applyConfig: "应用配置",
    restoreConfig: "撤销恢复",
    matchingEndpoints: "匹配的端点",
    noMatchingEndpoints: "没有找到匹配的端点（请确保已启用对应转换器的端点）",
    configError: "配置错误",
    applySuccess: "配置应用成功",
    applyFailed: "配置应用失败",
    restoreSuccess: "配置已撤销并恢复",
    restoreFailed: "配置恢复失败",
    // 错误消息
    errors: {
