...
import {
  ...
  loadStatsByPeriod,
  getCurrentPeriod,
  updateEndpointStatsCache,
  updateTotalStatsCache,
...
import {
  ...
  switchEndpointViewMode,
  initEndpointViewMode,
  updateEndpointStatsIncremental,
...
function handleStatsUpdate(data) {
  ...
  if (totalStats) {
    ...
    if (successRequestsEl)
      successRequestsEl.textContent = totalStats.requests - totalStats.errors;
    if (failedRequestsEl) failedRequestsEl.textContent = totalStats.errors;
    if (totalTokensEl)
      totalTokensEl.textContent = formatTokens(
        totalStats.inputTokens + totalStats.outputTokens,
      );
    if (totalInputTokensEl)
      totalInputTokensEl.textContent = formatTokens(totalStats.inputTokens);
    if (totalOutputTokensEl)
      totalOutputTokensEl.textContent = formatTokens(totalStats.outputTokens);
  }
